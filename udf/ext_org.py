...
for row in sys.stdin:
  # Find phrases that are continuous words tagged with ORG.
  ...
  while start_index < len(words):
    # Checking if there is a ORG phrase starting from start_index
    index = start_index
    while index < len(words) and ner_tags[index] == "ORG":
      index += 1
